...
import uk.fls.main.screens.gui.Slider;
import uk.fls.main.util.ArtFilter;
import uk.fls.main.util.Tile;
import uk.fls.main.util.pallet.Pallet;
import uk.fls.main.util.pallet.PalletManager;
...
public class PaintScreen extends Screen {
	...
	public void postInit() {
		...
		this.colorSliders = new Slider[3];
		int[] c = new int[]{255 << 16, 255 << 8, 255};
		for (int i = 0; i < 3; i++) {
			this.colorSliders[i] = new Slider();
			this.colorSliders[i].sliderColor = c[i];
		}
		initialise();
	}
	...
	public void update() {
		if (this.game.hasFocus()) {
			...
			if (this.state == 1) {
				...
				// This section checks to see if the mouse is over the pallet
				// area
				int paxm = 8;
				...
				if (mx > paxm && my > paym && mx < paxM && my < payM) {
					...
					if (this.input.leftMouseButton.justClicked()) {
						...
						if (newIndex > this.currentPallet.colors.length - 1)
							newIndex = this.currentPallet.colors.length - 1;
						this.currentColorIndex = newIndex;
					}
				}

				// This section will check to see if the mouse is over the load
				// and save buttons
				...
				if (mx > tpxm && my > tpym && mx < tpxM && my < tpyM) {
					...
					if (this.input.leftMouseButton.justClicked()) {
						...
						if (!(newToolIndex >= this.tools.size() || newToolIndex < 0))
							this.currentTool = this.tools.get(newToolIndex);
					}
				}

				// This section checks to see if the mouse is over the minus
				// size button
				...
				for (int i = 0; i < this.colorSliders.length; i++) {
					this.colorSliders[i].update(this.input.leftMouseButton.isHeld(), mx, my);
				}
			}
	...
	public void render(Graphics g) {
		...
		if (this.state == 1) {
			...
			drawCurrentSize();
		} else if (this.state == 0) {// Pallet things
			int xd = 4;
			int yd = 14;
			renderBorder(xd, yd, 16, 4);
			for (int i = 0; i < this.currentPallet.colors.length; i++) {
				...
				for (int pp = 0; pp < 8 * 8; pp++) {
					this.r.setPixel(xo + (pp % 8), yo + (pp / 8), c);
				}
			}
			...
			int yo = 16;
			for (int i = 0; i < 3; i++) {
				this.colorSliders[i].render(r, xo, yo + (i * 16));
				renderString("" + this.colorSliders[i].getValue(), xo + 88, yo + (i * 16));
			}
			
			int rgb = this.r.makeRGB(this.colorSliders[0].getValue(), this.colorSliders[1].getValue(), this.colorSliders[2].getValue());
			renderBorder(16, 2, 5, 5);
			int dx = 8 * 5;
		...
		for (int i = 0; i < this.currentPallet.colors.length; i++) {
			...
			for (int j = 0; j < s * s; j++) {
				...
				if (i == this.currentColorIndex) {
					if (dx == 0 || dy == 0 || dy == s - 1 || dx == s - 1)
						this.r.setPixel(xoff + (tx * s) + dx, yoff + (ty * s) + dy, 255 << 16);
				}
			}
		}
	}
	...
	private void drawTools() {
		...
		for (int i = 0; i < this.tools.size(); i++) {
			...
			for (int j = 0; j < 8 * 8; j++) {
				...
				if (this.currentTool == t) {// Draws a red box around the
	...
	private void save() {
		...
		if (out == JFileChooser.APPROVE_OPTION) {
			...
			if(name.lastIndexOf(".") == -1){
				fo = PluginManager.instance.getFileOperationsWithExtension(".art");
			}else{
				fo = PluginManager.instance.getFileOperationsWithExtension(file.getName().substring(file.getName().lastIndexOf(".")));
			}
	...
	private void load() {
		...
		if (out == JFileChooser.APPROVE_OPTION) {
			...
			if (fileName.indexOf(".") != -1) {// Has extension
				...
				ext = ext.trim();
				Tile[] t = PluginManager.instance.getFileOperationsWithExtension(ext)
						.load(FileIO.instance.loadFile(sel.getAbsolutePath()));
			...
			for (int yy = -1; yy <= h; yy++) {
				...
				for (int i = 0; i < 8 * 8; i++) {
					...
					if (tx != 1) {
						...
						this.r.setPixel(x + (xx * 8) + dx, y + (yy * 8) + dy,
								(this.r.makeRGB(123, 164, 255) & 0xFEFEFE) >> 1);
					}
				}
			}
		}
	...
	private void chageSize(int nSize) {
		...
		int tileDrawSize = this.sv.w / numTiles; // The number of new tiles we
													// want
	...
	private void initPallets(){
		PalletManager.instance.init();
		setPallet(PalletManager.instance.getPallets().get(1));
	}
	...
	private JFileChooser getSaveLoadDialog() {
		...
		for (int i = 0; i < PluginManager.instance.getFileManagerList().length; i++) {
			jfc.addChoosableFileFilter(new ArtFilter(PluginManager.instance.getFileManagerList()[i],
					PluginManager.instance.getFileManagerDescList()[i]));
		}
		return jfc;
	}
